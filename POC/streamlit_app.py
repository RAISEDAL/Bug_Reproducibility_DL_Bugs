...
def main():
    ...
    page = st.radio(
        "Select a page",
        [
            "Stack Overflow Edit Actions Recommender",
            "Custom Edit Actions Recommender",
    ...
    if page == "Our findings on Deep Learning Bug Reproducibility":
        home_page()
    elif page == "Stack Overflow Edit Actions Recommender":
        recommendation_page()
    elif page == "Custom Edit Actions Recommender":
        custom_recommendation_page()
...
def custom_recommendation_page():
    ...
    description = st.text_area("Enter the bug description:")
    code_snippet = st.text_area("Enter the code snippet:")
    question_tags = st.text_input("Enter the tags for the bug in comma-separated fashion (e.g., \"loss-function, activation-function\"):")
    question_tags = question_tags.split(",")
    question_tags = [tag.strip() for tag in question_tags]
    
    if st.button("Recommend Edit Actions and Critical Information"):
        recommend_edit_actions(description, code_snippet, tags, model, critical_information, edit_actions, question_tags)

def recommend_edit_actions(description, code_snippet, tags, model, critical_information, edit_actions, question_tags):
    with st.spinner("Retrieving the type of bug from the description and tags"):
        type_of_bug = "-"
        tag_to_bug_type = {
            "model": "model",
            "training": "training",
